...
for hotspot_azimuth_offset in range(0, 19):
            
    try:    
        
        """ Evaluate the simulataion results """
        Sim_parser = Simulation_Parser(parent_directory + "Reference_simulations\\Hotspot_Reference_Simulation_{}".format(hotspot_azimuth_offset) + "\\" + metric)
        I_Intensity, Q_Intensity, U_Intensity, _, _, _, _, _ = Sim_parser.get_plottable_sim_data()
        Total_flux = Sim_parser.get_total_flux(Units.SGRA_DISTANCE_GEOMETRICAL, unit = "mJy")
        Polarized_flux = Sim_parser.get_polarized_flux(Units.SGRA_DISTANCE_GEOMETRICAL, unit = "mJy")
        
        Total_intensity = Total_intensity + I_Intensity
        
        Total_flux_list.append(Total_flux)
        Polarized_flux_list.append(Polarized_flux)
        Time.append(float(Sim_parser.Simulation_metadata["Observervation Time [M]"]))
        ...
        Net_Q.append(sum((Q_Intensity).flatten()) * abs(axes_limits[0] - axes_limits[1]) * abs(axes_limits[2] - axes_limits[3]) / X_resolution / Y_resolution / Units.RAD_TO_MICRO_AS**2 * 1e3)
        Net_U.append(sum((U_Intensity).flatten()) * abs(axes_limits[0] - axes_limits[1]) * abs(axes_limits[2] - axes_limits[3]) / X_resolution / Y_resolution / Units.RAD_TO_MICRO_AS**2 * 1e3)
  
        Rel_Q.append(sum((Q_Intensity).flatten()) / sum((I_Intensity).flatten()))
        Rel_U.append(sum((U_Intensity).flatten()) / sum((I_Intensity).flatten()))
        
        LP_Fraction.append(Polarized_flux / Total_flux)
        EVPA.append(0.5 * np.arctan2(sum((U_Intensity).flatten()), sum((Q_Intensity).flatten())))
...
Intensity_plot.plot(Total_flux_list, "r--")
# Intensity_plot.plot(Time, Total_flux_w_rings, "b-")
Intensity_plot.plot(Polarized_flux_list, "k-")
