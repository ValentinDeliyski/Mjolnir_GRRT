...
class Polarization_visuzlier():
    ...
    def __init__(self, Sim_path: str, Fig_output_path: str, fontsize: int = 16) -> None:
        ...
        _, _, _, _, Numerical_Disk_redshift, Numerical_Disk_flux, Numerical_Pol_vec_x, Numerical_Pol_vec_y, _, _ = self.Numerical_sim_parser.get_plottable_sim_data()
        ...
        _, _, _, _, Kerr_Disk_redshift, Kerr_Disk_flux, Kerr_Pol_vec_x, Kerr_Pol_vec_y, _, _ = self.Kerr_sim_parser.get_plottable_sim_data()
        ...
        self.Flux_data = [Numerical_Disk_flux, Kerr_Disk_flux]
        self.Observed_flux_data = array([Numerical_Disk_redshift**4 * (Numerical_Pol_vec_x**2 + Numerical_Pol_vec_y**2), Kerr_Disk_redshift**4 * (Kerr_Pol_vec_x**2 + Kerr_Pol_vec_y**2)])
    ...
    def Plot_polarization_ticks(self, Pixel_skip_step: int, Scale_factor: float = 1) -> None:
        ...
        Main_Figure = plt.figure(figsize = (25, 6.5))
        ...
        Numerical_subplot.set_title(r"Numerical Configuration I$^{0}_{0.01}$", fontsize = self.fontsize)
        ...
        # Kerr_subplot.set_ylabel(r"$y\,[M_{\text{ADM}}]$")
        ...
        Colorbar_norm = colors.Normalize(vmin = float(min(self.Observed_flux_data.flatten())), vmax = float(max(self.Observed_flux_data.flatten())))
   
        for _, (Flux, Redshift, Pol_x, Pol_y, X_coord, Y_coord, Axes_limits, Subplot) in enumerate(zip(self.Flux_data, self.Redshift_data, self.Pol_vec_x_data, self.Pol_vec_y_data, 
                                                                                                       self.X_coords_data, self.Y_coords_data, self.Axes_limits_data, Subplot_list)):
        
            Colormap = colormaps['plasma']
            
            Colormap.set_bad((0, 0, 0, 1))
            data_to_plot = Redshift**4 * (Pol_x**2 + Pol_y**2)
            data_to_plot[data_to_plot == 0] = nan
            
            Disk_image = Subplot.imshow(data_to_plot, extent = tuple(Axes_limits), cmap = Colormap, interpolation = "nearest", norm = Colorbar_norm)
            
            colorbar: Colorbar = Main_Figure.colorbar(Disk_image, ax = Subplot, fraction = 0.046, pad = 0.04, orientation = "vertical", extend = "both")
            colorbar.set_label(r"Intensity [-]", fontsize = self.fontsize)
            # colorbar.ax.spines["bottom"].set_color("0.5")
            # (r"Intensity [-]", fontsize = self.fontsize)
            ...
            Subplot.tick_params(which = 'minor', length = 4)
            Subplot.tick_params(which = 'major', length = 8)

            Pol_tick_scale = Scale_factor / sqrt(Redshift**4 * (Pol_x**2 + Pol_y**2) + 1e-10)
        ...
        Numerical_intensity = self.Redshift_data[self.Metric_enums.Numerical.value][self.Slice_idx]**4 * (self.Pol_vec_x_data[self.Metric_enums.Numerical.value][self.Slice_idx]**2 + self.Pol_vec_y_data[self.Metric_enums.Numerical.value][self.Slice_idx]**2)
        Kerr_intensity = self.Redshift_data[self.Metric_enums.Kerr.value][self.Slice_idx]**4 * (self.Pol_vec_x_data[self.Metric_enums.Kerr.value][self.Slice_idx]**2 + self.Pol_vec_y_data[self.Metric_enums.Kerr.value][self.Slice_idx]**2)
        
        Pol_intensity_plot.plot(self.X_coords_data[self.Metric_enums.Numerical.value][self.Slice_idx], Numerical_intensity, color = "C0")
        Pol_intensity_plot.plot(self.X_coords_data[self.Metric_enums.Kerr.value][self.Slice_idx], Kerr_intensity, color = "m")
        ...
        Pol_intensity_plot.set_aspect(0.91 * (max(self.X_coords_data[self.Metric_enums.Numerical.value][self.Slice_idx].flatten()) - min(self.X_coords_data[self.Metric_enums.Numerical.value][self.Slice_idx].flatten())) / (1.1 * (Intensity_max - Intensity_min)))
        ...
        Pol_intensity_plot.tick_params(which = 'minor', length = 4)
        Pol_intensity_plot.tick_params(which = 'major', length = 8) 
               
        Pol_intensity_plot.legend([r"Numerical Configuration I$^{0}_{0.01}$", "Kerr analog"], loc = "upper right")
        ...
        EVPA_plot.plot(Left_Numerical_X_coord, Left_Numerical_EVPA, color = "C0")
        EVPA_plot.plot(Right_Numerical_X_coord, Right_Numerical_EVPA, color = "C0")
        ...
        EVPA_plot.plot(Left_Kerr_X_coord, Left_Kerr_EVPA, "m")
        EVPA_plot.plot(Right_Kerr_X_coord, Right_Kerr_EVPA, "m")
        ...
        if EVPA_min < 0:
            ...
            EVPA_plot.set_aspect(0.91 * (X_coord_max - X_coord_min) / (1.1 * (EVPA_max - EVPA_min)))
            
        else:
            EVPA_plot.set_ylim(0.9 * EVPA_min, 1.1 * EVPA_max)
            EVPA_plot.set_aspect(0.91 * (X_coord_max - X_coord_min) / (1.1 * EVPA_max - 0.9 * EVPA_min))      
        ...
        EVPA_plot.tick_params(which = 'minor', length = 4)
        EVPA_plot.tick_params(which = 'major', length = 8) 
        ...
        Main_Figure.savefig(self.Output_path + "\\Whole_disk_plot_B_{}_{}_{}.png".format(self.B_r,
                                                                                         self.B_theta,
                                                                                         self.B_phi), bbox_inches = 'tight')
    ...
    def get_image_at_fixed_source_radius_numerical_only(self, r_source: float, tolerance: float, length_step: float, Scale_factor: float, axis_limits: list[float]) -> None:
        ...
        Image_azimuth = Image_azimuth[Sorting_idx] + 2 * pi
        ...
        _, _, _, _, Disk_redshift, _, Pol_vec_x, Pol_vec_y, _, _ = self.Numerical_sim_parser.get_plottable_sim_data()
        
        Colormap = colormaps['plasma']
            
        Colormap.set_bad((0, 0, 0, 1))
        data_to_plot = Disk_redshift**4 * (Pol_vec_x**2 + Pol_vec_y**2)
        data_to_plot[data_to_plot == 0] = nan
        ...
        Subplot.imshow(data_to_plot, extent = tuple(Axes_limits), cmap = Colormap, interpolation = "nearest", vmin = 0)
    ...
    def get_image_at_fixed_source_radius(self, r_source: float, tolerance: float, length_step: float, Scale_factor: float, axis_limits: list[float], Tick_visualization: bool, Curve_visualization: bool, Subplots: list) -> None:
        ...
        if Tick_visualization:
            ...
            Colormap.set_bad((0, 0, 0, 1))
            data_to_plot = Kerr_Disk_redshift**4 * (Kerr_Pol_vec_x**2 + Kerr_Pol_vec_y**2)
            data_to_plot[data_to_plot == 0] = nan
            
            Kerr_axes_limits = self.Kerr_sim_parser.Simulation_metadata["Observation Window Dimentions (-X,+X,-Y,+Y) [M]"].split(",")
            Kerr_axes_limits = array([float(Limit) / self.Solution_mass for Limit in Kerr_axes_limits])
            
            Subplots[1].imshow(data_to_plot, extent = tuple(Kerr_axes_limits), cmap = Colormap, interpolation = "nearest", vmin = 0)
            ...
            Colormap.set_bad((0, 0, 0, 1))
            data_to_plot = Numerical_Disk_redshift**4 * (Numerical_Pol_vec_x**2 + Numerical_Pol_vec_y**2)
            data_to_plot[data_to_plot == 0] = nan
            
            Numerical_axes_limits = self.Numerical_sim_parser.Simulation_metadata["Observation Window Dimentions (-X,+X,-Y,+Y) [M]"].split(",")
            Numerical_axes_limits = array([float(Limit) / self.Solution_mass for Limit in Numerical_axes_limits])
            
            Subplots[0].imshow(data_to_plot, extent = tuple(Numerical_axes_limits), cmap = Colormap, interpolation = "nearest", vmin = 0)
        ...
        if Curve_visualization:
            ...
            Subplots[1].tick_params(which = 'minor', length = 4)
            Subplots[1].tick_params(which = 'major', length = 8) 
            ...
            Subplots[0].set_title(r"Numerical Configuration I$^{0}_{0.01}$", fontsize = self.fontsize)
            
            Subplots[0].minorticks_on()
            Subplots[0].tick_params(axis = "both", direction = "out")
            Subplots[0].tick_params(which = 'minor', length = 4)
            Subplots[0].tick_params(which = 'major', length = 8) 
            
            """" ================================================================================================================================================== """
            
            Subplots[2].plot(Image_azimuth - Image_azimuth[0], Numerical_redshift**4 * (Numerical_Pol_x**2 + Numerical_Pol_y**2))
            Subplots[2].plot(Image_azimuth - Image_azimuth[0], Kerr_redshift**4 * (Kerr_Pol_x**2 + Kerr_Pol_y**2), "m")
            ...
            Subplots[2].tick_params(which = 'minor', length = 4)
            Subplots[2].tick_params(which = 'major', length = 8) 

            Subplots[2].set_xlim(0, 2 * pi)
            Subplots[2].set_ylabel(r"Intensity [-]", fontsize = self.fontsize)
            Subplots[2].legend([r"Numerical Configuration I$^{0}_{0.01}$", "Kerr analog", ])
             
            """" ================================================================================================================================================== """
            
            Subplots[3].plot(Image_azimuth - Image_azimuth[0], arctan(-Numerical_Pol_x / Numerical_Pol_y))
            Subplots[3].plot(Image_azimuth - Image_azimuth[0], arctan(-Kerr_Pol_x / Kerr_Pol_y), "m")
            ...
            Subplots[3].tick_params(which = 'minor', length = 4)
            Subplots[3].tick_params(which = 'major', length = 8) 

            Subplots[3].set_xlim(0, 2 * pi)
            Subplots[3].set_ylabel(r"EVPA [rad]", fontsize = self.fontsize)
            # Subplots[3].legend(["Kerr analog", r"Numerical Configuration V$^{0}_{0.2}$"])
            
            """" ================================================================================================================================================== """
            
            Subplots[4].plot(Image_azimuth - Image_azimuth[0], Kerr_redshift**4 * (Kerr_Pol_x**2 + Kerr_Pol_y**2) - Numerical_redshift**4 * (Numerical_Pol_x**2 + Numerical_Pol_y**2))
            Delta_I_max = max(Kerr_redshift**4 * (Kerr_Pol_x**2 + Kerr_Pol_y**2) - Numerical_redshift**4 * (Numerical_Pol_x**2 + Numerical_Pol_y**2))
            Delta_I_min = min(Kerr_redshift**4 * (Kerr_Pol_x**2 + Kerr_Pol_y**2) - Numerical_redshift**4 * (Numerical_Pol_x**2 + Numerical_Pol_y**2))
            ...
            Subplots[4].tick_params(which = 'minor', length = 4)
            Subplots[4].tick_params(which = 'major', length = 8) 
            
            Subplots[4].set_xlim(0, 2 * pi)
            Subplots[4].set_ylabel(r"$\Delta I$ [-]", fontsize = self.fontsize)
            Subplots[4].set_xlabel(r"Image azimuth [rad]", fontsize = self.fontsize)
            
            """" ================================================================================================================================================== """
            
            Subplots[5].plot(Image_azimuth - Image_azimuth[0], arctan(-Kerr_Pol_x / Kerr_Pol_y) - arctan(-Numerical_Pol_x / Numerical_Pol_y))
            Delta_EVPA_max = max(arctan(-Kerr_Pol_x / Kerr_Pol_y) - arctan(-Numerical_Pol_x / Numerical_Pol_y))
            Delta_EVPA_min = min(arctan(-Kerr_Pol_x / Kerr_Pol_y) - arctan(-Numerical_Pol_x / Numerical_Pol_y))
            ...
            Subplots[5].tick_params(which = 'minor', length = 4)
            Subplots[5].tick_params(which = 'major', length = 8) 
        
            Subplots[5].set_xlim(0, 2 * pi)
            Subplots[5].set_ylabel(r"$\Delta$EVPA [rad]", fontsize = self.fontsize)
...
if __name__ == "__main__":
    ...
    Sim_path = "E:\\Numerical_metric_runs\\Zero_curvature\\Config_I_70_deg_B_0.87_0.0_0.5_min_order_0_max_order_0"
    
    Visualizer_instance = Polarization_visuzlier(Sim_path = Sim_path, fontsize = 16, Fig_output_path = "E:\\Numerical_metric_runs\\Zero_curvature\\Figures\\Config_I_70_deg_min_order_0_max_order_0")
    ...
    Visualizer_instance.get_image_at_fixed_source_radius(r_source = 2.1509050411976376, 
                                                         tolerance = 0.01, 
                                                         length_step = 0.21, 
                                                         Scale_factor = 0.75, 
                                                         axis_limits = [-4, 5, -2, 5],
