...
def JNW_mb_orbit_condition(r: float, gamma: float) -> float:
...
for gamma in gamma_range:
    ...
    try:
        solution = brentq(JNW_mb_orbit_condition, args = gamma, a = 4, b = 10, xtol= 1e-10)

        if solution > (2 / gamma) + 1e-10:
            r_mb.append(solution)
            gamma_plot_r_mb.append(gamma)
...
plt.text(gamma_plot_r_ISCO_inner[int(len(r_ISCO_inner) / 4)], r_ISCO_inner[int(len(r_ISCO_inner) / 4)] + 0.1, r'$r_{ISCO}^-$', ha='left', va='bottom',
...
plt.text(gamma_range[int(len(r_ISCO_outer) / 2)], r_ISCO_outer[int(len(r_ISCO_outer) / 2)], r'$r_{ISCO}^+$', ha='left', va='bottom',
         transform_rotates_text=True, rotation=angle, rotation_mode='anchor', fontsize = 16)
...
dx    = gamma_plot_r_mb[int(len(r_mb) / 1.5)] - gamma_plot_r_mb[int(len(r_mb) / 1.5 - 1)]
dy    = r_mb[int(len(r_mb) / 1.5)] - r_mb[int(len(r_mb) / 1.5 - 1)]
angle = np.rad2deg(np.arctan2(dy, dx))

plt.text(gamma_range[int(len(r_ISCO_outer) / 2)], r_mb[int(len(r_mb) / 1.5)] + 0.15, r'$r_\text{mb}$', ha='left', va='bottom',
         transform_rotates_text=True, rotation=angle, rotation_mode='anchor', fontsize = 16)

plt.plot(gamma_plot_r_mb, r_mb)

plt.plot(gamma_plot_r_mb[0], r_mb[0], "o", color = "blue")

# ================================================================================================================================================= #

dx    = gamma_plot_r_ph[int(len(r_mb) / 1.5)] - gamma_plot_r_ph[int(len(r_mb) / 1.5 - 1)]
dy    = r_ph[int(len(r_mb) / 1.5)] - r_ph[int(len(r_mb) / 1.5 - 1)]
angle = np.rad2deg(np.arctan2(dy, dx))
