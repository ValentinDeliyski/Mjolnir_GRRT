...
if __name__  == "__main__":
    ...
    for gamma in np.linspace(0, 3, 31):

        if gamma == 0:
            gamma = 0.01

        r_image_in_WH.append(parse_file("Wormhole_r_4.5_gamma_{}_20.0_deg_indirect.csv".format(round(gamma, 2))))
        r_image_out_WH.append(parse_file("Wormhole_r_500_gamma_{}_20.0_deg_indirect.csv".format(round(gamma, 2))))
    ...
    for gamma_value in range(31):

        if gamma_value % 9 == 0:

            Color_range = cmap(gamma_value / 30) 

            Inner_plot.plot(x_axis, r_image_in_WH[gamma_value], color = Color_range)
            Inner_plot.plot(x_axis, r_image_out_WH[gamma_value], color = Color_range, label = r"$\gamma = ${}".format(round(gamma_value * 3 / 30,2)))
