...
for simple_multiplier in [1]:

    for tv_multiplier in [1, 10, 100]:
        
        for l1_multiplier in [0, 1, 10, 100]:
            
            for fwhm in [40, 50, 60]:
                
                for total_flux in [0.5, 0.6, 0.7]:
                    
                    for FOV_mult in [1, 1.2]:

                        Simulation_name = "Sim_Paper_1_follow_up/run_4/Sch/Sch_EHT2022_simple_{}_tv_{}_l1_{}_fwhm_{}_tot_flux_{}_fov_mult_{}".format(simple_multiplier, tv_multiplier, l1_multiplier, fwhm, total_flux, FOV_mult)
                        
                        print("Running " + Simulation_name + "...")
                        
                        if not os.path.isfile('Ehtim_Output_Data/' + Simulation_name + '/Results_blur.txt'):
                            try:
                        ...
                        im  = eh.image.load_txt('C:\\Users\\Valur\\Documents\\Repos\\Mjolnir_GRRT\\Utilities\\Ehtim\\Ehtim_Input_Data\\Sch_data_for_ehtim_230.csv')
                        eht = eh.array.load_txt('C:\\Users\\Valur\\Documents\\Repos\\Mjolnir_GRRT\\Utilities\\Ehtim\\Ehtim_Input_Data\\arrays\\EHT2022.txt')
                        ...
                        npix = 128
                        fov = FOV_mult * im.fovx()
                        zbl = total_flux      # total flux
                        prior_fwhm = fwhm * eh.RADPERUAS # Gaussian size in microarcssec
                        ...
                        imgr  = eh.imager.Imager(obs, gaussprior, gaussprior, flux,
                                                ...
                                                reg_term={'simple': simple_multiplier * 1,'tv2': tv_multiplier * 1, 'flux':100, 'cm':100, "l1": l1_multiplier},
                        ...
                        imgr.reg_term_next = {'simple': simple_multiplier * 1,'tv2': tv_multiplier * 50,'flux':50,'cm':50, "l1": l1_multiplier}
                        imgr.maxi_next  = 3000
                        imgr.make_image(show_updates=False)
                        ...
                        imgr.reg_term_next = {'simple': simple_multiplier * 1,'tv2': tv_multiplier * 100,'flux': 10,'cm': 10, "l1": l1_multiplier}
                        imgr.maxi_next  = 4000
                        imgr.make_image(show_updates=False)
                        ...
                        imgr.reg_term_next  = {'simple': simple_multiplier * 1,'tv2': tv_multiplier * 500,'flux': 1,'cm': 1, "l1": l1_multiplier}
                        imgr.maxi_next  = 4000
                        imgr.make_image(show_updates=False)
