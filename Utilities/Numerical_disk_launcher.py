...
class Simulation:
    ...
    def __init__(self, resolution, inclination):
        ...
        self.Nominal_Image_x_max = 25
        self.Nominal_Image_y_max = 25
    
        self.Simulation_configurator.observer.Image_y_min = {"Value": -25, "Unit": "[M]"}
        self.Simulation_configurator.observer.Image_y_max = {"Value":  25, "Unit": "[M]"}
        self.Simulation_configurator.observer.Image_x_min = {"Value": -25, "Unit": "[M]"}
        self.Simulation_configurator.observer.Image_x_max = {"Value":  25, "Unit": "[M]"}
        ...
        self.Simulation_configurator.geodesic_integrator.min_upper_stepsize = {"Value": 0.01, "Unit": "[-]"}
        self.Simulation_configurator.geodesic_integrator.max_step_b_coeff = {"Value": 0.0001, "Unit": "[-]"}
        self.Simulation_configurator.geodesic_integrator.Max_step_in_emission_medium = {"Value": 1e-6, "Unit": "[-]"}
    ...
    def Combine_segmented_output_files(self):
            ...
            with open(self.Output_folder + "\\Numerical_results\\All_Segments_Results.txt", "w") as file:
                ...
                if not NT_model_active:
                    ...
                    file.write("\nImage X Coord [M],Image Y Coord [M],Synchotron Intensity I [Jy/sRad],Synchotron Intensity Q [Jy/sRad],Synchotron Intensity U [Jy/sRad],Synchotron Intensity V [Jy/sRad],Celestial Sphere Crossing Theta [Rad],Celestial Sphere Crossing Phi [Rad]\n")
                
                    for Tuple_to_write in zip(X_coords, Y_coords, I_Intensity, Q_Intensity, U_Intensity, V_Intensity, Celestial_theta, Celestial_phi):
...
if __name__ == "__main__":
    ...
    for inc in [70, 45, 20]:
        ...
        Sim_instance = Simulation(inclination = inc, resolution = 1024)
