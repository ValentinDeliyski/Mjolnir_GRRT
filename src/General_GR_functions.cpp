...
std::complex<double> get_Penrose_Walker_constant(const double* const Global_State_Vector, const Simulation_Context_type* const p_Sim_Context, const std::complex<double>* const Polarization_Vector) {

    double Contravariant_momentum[4]{};

    Metric_type s_Metric = p_Sim_Context->p_Spacetime->get_global_metric(Global_State_Vector);
    Manipulate_index(&s_Metric, Global_State_Vector + e_p_t, Contravariant_momentum, Raise_index);
    ...
    std::complex<double> Kappa_2 = pow(s_Metric.Metric[e_theta][e_theta], 3.0 / 2) * sin(Global_State_Vector[e_theta]) * (p_theta * Polarization_Vector[e_phi] - p_phi * Polarization_Vector[e_theta]);
    ...
    if (Kerr == p_Sim_Context->p_Init_Conditions->Metric_parameters.e_Spacetime) {
        ...
        Kappa_1 = p_t * Polarization_Vector[e_r] - p_r * Polarization_Vector[e_t] + Spin * sin(Global_State_Vector[e_theta]) * sin(Global_State_Vector[e_theta]) * (p_r * Polarization_Vector[e_phi] - p_phi * Polarization_Vector[e_r]);
        Kappa_2 = sin(Global_State_Vector[e_theta]) * ((Global_State_Vector[e_r] * Global_State_Vector[e_r] + Spin * Spin) * (p_phi * Polarization_Vector[e_theta] - p_theta * Polarization_Vector[e_phi]) - Spin * (p_t * Polarization_Vector[e_theta] - p_theta * Polarization_Vector[e_t]));

        return (Kappa_1 - complex_i * Kappa_2) * (Global_State_Vector[e_r] - complex_i * Spin * cos(Global_State_Vector[e_theta]));
